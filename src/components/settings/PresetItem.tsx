...
export function PresetItem({ preset, onEdit, onDelete, onToggleFavorite }: PresetItemProps) {
  ...
  return (
    <div className="bg-card px-3 py-2 rounded-lg flex items-center gap-3 overflow-hidden border border-border/80 shadow-[var(--shadow-soft)]">
      ...
      <div className="flex-1 min-w-0">
        <div className="font-semibold text-foreground truncate">{preset.name}</div>
      ...
      <button onClick={onDelete} className="p-1.5 tap-target touch-feedback text-destructive hover:text-destructive/90 flex-shrink-0">
